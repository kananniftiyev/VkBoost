#include "vkxtra/shader_compiler.hpp"
...
bool vkxtra::shader::shader_compiler::changeToProjectPath(const std::vector<fs::path> &markers)
{
    while (true)
    {
...
fs::path vkxtra::shader::shader_compiler::compileShader(fs::path path)
{
...
bool vkxtra::shader::shader_compiler::isShaderModified(fs::path path, mINI::INIStructure &ini)
{
...
std::string vkxtra::shader::shader_compiler::fileLastWriteTimeStr(const fs::path &path)
{
...
vkxtra::shader::shader_compiler::shader_compiler(std::string file_path) : _folder_path(std::move(file_path))
{
